...
export default function Particles({
    ...
    const drawCircle = (circle: Circle, update = false) => {
        if (context.current) {
            ...
            // distinct optimization: direct draw without modifying context matrix
            const finalX = x + translateX;
            const finalY = y + translateY;

            context.current.beginPath();
            context.current.arc(finalX, finalY, size, 0, 2 * Math.PI);
            context.current.fillStyle = color;
            context.current.globalAlpha = alpha;
            context.current.fill();

            // Reset globalAlpha not transform
    ...
    const animate = () => {
        ...
        circles.current.forEach((circle: Circle, i: number) => {
            ...
            // Optimization: Find min without reducing array which allocates
            let closestEdge = edge[0];
            if (edge[1] < closestEdge) closestEdge = edge[1];
            if (edge[2] < closestEdge) closestEdge = edge[2];
            if (edge[3] < closestEdge) closestEdge = edge[3];

            // Remove toFixed/parseFloat string churn
